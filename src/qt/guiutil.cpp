...
{
    widget->setMaxLength(TokenOfDigitalMoneyAddressValidator::MaxAddressLength);
    widget->setValidator(new TokenOfDigitalMoneyAddressValidator(parent));
...
bool parseTokenOfDigitalMoneyURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("TokenOfDigitalMoney"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TokenOfDigitalMoneyUnits::parse(TokenOfDigitalMoneyUnits::DMB, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTokenOfDigitalMoneyURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to TokenOfDigitalMoney:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rev://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "TokenOfDigitalMoney:");
    }
    QUrl uriInstance(uri);
    return parseTokenOfDigitalMoneyURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TokenOfDigitalMoney.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TokenOfDigitalMoney.lnk
...
{
    return GetAutostartDir() / "TokenOfDigitalMoney.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a TokenOfDigitalMoney.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TokenOfDigitalMoney\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the TokenOfDigitalMoney app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add TokenOfDigitalMoney app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("TokenOfDigitalMoney-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  TokenOfDigitalMoney-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("TokenOfDigitalMoney-Qt"));
