...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", TokenOfDigitalMoneyUnits::DMB);
    ...
    if (!settings.contains("nAnonymizeTokenOfDigitalMoneyAmount"))
        settings.setValue("nAnonymizeTokenOfDigitalMoneyAmount", 1000);
    nAnonymizeTokenOfDigitalMoneyAmount = settings.value("nAnonymizeTokenOfDigitalMoneyAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTokenOfDigitalMoneyAmount"))
        SoftSetArg("-anonymizeTokenOfDigitalMoneyamount", settings.value("nAnonymizeTokenOfDigitalMoneyAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTokenOfDigitalMoneyAmount:
            return QVariant(nAnonymizeTokenOfDigitalMoneyAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeTokenOfDigitalMoneyAmount:
            nAnonymizeTokenOfDigitalMoneyAmount = value.toInt();
            settings.setValue("nAnonymizeTokenOfDigitalMoneyAmount", nAnonymizeTokenOfDigitalMoneyAmount);
            emit AnonymizeTokenOfDigitalMoneyAmountChanged(nAnonymizeTokenOfDigitalMoneyAmount);
            break;
        default:
            break;
        }
