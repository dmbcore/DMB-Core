...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(TokenOfDigitalMoneyUnits::DMB)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseDarkTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = TokenOfDigitalMoneyUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(TokenOfDigitalMoneyUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(TokenOfDigitalMoneyUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(TokenOfDigitalMoneyUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(TokenOfDigitalMoneyUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelTotal->setText(TokenOfDigitalMoneyUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    //ui->labelWatchAvailable->setText(TokenOfDigitalMoneyUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    //ui->labelWatchStake->setText(TokenOfDigitalMoneyUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    //ui->labelWatchPending->setText(TokenOfDigitalMoneyUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    //ui->labelWatchImmature->setText(TokenOfDigitalMoneyUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    //ui->labelWatchTotal->setText(TokenOfDigitalMoneyUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    //ui->labelDMBtotalimg->setVisible(showWatchOnly);    // show watch-only balance separator line
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("DMB")
    updateDisplayUnit();
}
...
{
    TokenOfDigitalMoneyGUI::gotoHistoryPage_static();
}
