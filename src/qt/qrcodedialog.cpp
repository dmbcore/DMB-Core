...
{
    QString ret = QString("TokenOfDigitalMoney:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DMB unit input in lnReqAmount, we generate the URI with DMB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TokenOfDigitalMoneyUnits::format(TokenOfDigitalMoneyUnits::DMB, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
