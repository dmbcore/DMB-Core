...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "TokenOfDigitalMoney is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "TokenOfDigitalMoney is downloading blocks...");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 128/4; i++)
            ((unsigned int*)pdata)[i] = ByteTokenOfDigitalMoneyerse(((unsigned int*)pdata)[i]);
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "TokenOfDigitalMoney is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "TokenOfDigitalMoney is downloading blocks...");
