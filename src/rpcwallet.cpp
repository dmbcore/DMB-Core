...
#ifdef ENABLE_WALLET
        // Case 1: TokenOfDigitalMoney address and we have full public key:
        CTokenOfDigitalMoneyAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CTokenOfDigitalMoneyAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new TokenOfDigitalMoney address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"TokenOfDigitalMoney\"    (string) The new TokenOfDigitalMoney address\n"
            "\nExamples:\n"
    ...
    return CTokenOfDigitalMoneyAddress(keyID).ToString();
}


CTokenOfDigitalMoneyAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CTokenOfDigitalMoneyAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current TokenOfDigitalMoney address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"TokenOfDigitalMoney\"   (string) The account TokenOfDigitalMoney address\n"
            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"TokenOfDigitalMoney\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"TokenOfDigitalMoneyaddress\"  (string, required) The TokenOfDigitalMoney address to be associated with an account.\n"
    ...
    CTokenOfDigitalMoneyAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TokenOfDigitalMoney address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"TokenOfDigitalMoney\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"TokenOfDigitalMoney\"  (string, required) The TokenOfDigitalMoney address for account lookup.\n"
            "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"TokenOfDigitalMoney\"  (string) a TokenOfDigitalMoney address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CTokenOfDigitalMoneyAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CTokenOfDigitalMoneyAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"TokenOfDigitalMoney\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"TokenOfDigitalMoney\"  (string, required) The TokenOfDigitalMoney address to send to.\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"TokenOfDigitalMoney\",     (string) The TokenOfDigitalMoney address\n"
            "      amount,                 (numeric) The amount in DMB\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CTokenOfDigitalMoneyAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CTokenOfDigitalMoneyAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CTokenOfDigitalMoneyAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"TokenOfDigitalMoney\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"TokenOfDigitalMoney\"  (string, required) The TokenOfDigitalMoney address to use for the private key.\n"
    ...
    CTokenOfDigitalMoneyAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"TokenOfDigitalMoney\" ( minconf )\n"
            "\nReturns the total amount received by the given TokenOfDigitalMoney in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"TokenOfDigitalMoney\"  (string, required) The TokenOfDigitalMoney address for transactions.\n"
    ...
    // TokenOfDigitalMoney address
    CTokenOfDigitalMoneyAddress address = CTokenOfDigitalMoneyAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TokenOfDigitalMoney address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toTokenOfDigitalMoneyaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a TokenOfDigitalMoney address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toTokenOfDigitalMoneyaddress\"  (string, required) The TokenOfDigitalMoney address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CTokenOfDigitalMoneyAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TokenOfDigitalMoney address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The TokenOfDigitalMoney address is the key, the numeric amount in DMB is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CTokenOfDigitalMoneyAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CTokenOfDigitalMoneyAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid TokenOfDigitalMoney address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a TokenOfDigitalMoney address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of TokenOfDigitalMoney addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) TokenOfDigitalMoney address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"TokenOfDigitalMoney\"  (string) A TokenOfDigitalMoney address associated with the keys.\n"
    ...
    return CTokenOfDigitalMoneyAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CTokenOfDigitalMoneyAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CTokenOfDigitalMoneyAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CTokenOfDigitalMoneyAddress, tallyitem>::iterator it = mapTally.find(address);
...
{
    CTokenOfDigitalMoneyAddress addr;
    if (addr.Set(dest))
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"TokenOfDigitalMoney\",    (string) The TokenOfDigitalMoney address of the transaction. Not present for \n"
            ...
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in DMB. This is negative and only available for the \n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"TokenOfDigitalMoney\",    (string) The TokenOfDigitalMoney address of the transaction. Not present for move transactions (category = move).\n"
            ...
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in DMB. This is negative and only available for the 'send' category of transactions.\n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in DMB\n"
            ...
            "      \"address\" : \"TokenOfDigitalMoney\",   (string) The TokenOfDigitalMoney address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in DMB\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending TokenOfDigitalMoney\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"TokenOfDigitalMoneyaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; TokenOfDigitalMoney server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new TokenOfDigitalMoney stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid TokenOfDigitalMoney stealth address."));
        return result;
    };
...
Value cclistcoins(const Array& params, bool fHelp)
{
        ...
        BOOST_FOREACH(const COutput& out, vCoins)
    {
                ...
                coutput.push_back(Pair("Address", CTokenOfDigitalMoneyAddress(outputAddress).ToString()));
