...
int nAnonymizeTokenOfDigitalMoneyAmount = 1000;
...
#else
    const char* pszModule = "TokenOfDigitalMoney";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DMB
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DMB
    // Mac: ~/Library/Application Support/DMB
    // Unix: ~/.DMB
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TokenOfDigitalMoney.conf"));
...
void BuildConfigFile()
{
    // TODO: remove boost in favor of C++11 file lookup/handling
    boost::filesystem::path ConfPath;
    ConfPath = GetDataDir() / "TokenOfDigitalMoney.conf";
    FILE* ConfFile = fopen(ConfPath.string().c_str(), "w");
...
void InitializeConfigFile()
{
    ...
    if (!streamConfig.good())
    {
        // Create TokenOfDigitalMoney.conf
        BuildConfigFile();
        // Then read it...
        ReadConfigFile();
    } else {
        // Read TokenOfDigitalMoney.conf
        ReadConfigFile();
    }
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "TokenOfDigitalMoneyd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong TokenOfDigitalMoney will not work properly.");
