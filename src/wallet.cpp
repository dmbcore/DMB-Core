...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CTokenOfDigitalMoneyAddress(redeemScript.GetID()).ToString();
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough MNengine non-denominated funds for this transaction that are not equal 1000 DMB.");
                    }
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-TokenOfDigitalMoney-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CTokenOfDigitalMoneyAddress addr(ckid);
        ...
        if (fDebug)
        {
            ...
            CTokenOfDigitalMoneyAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CTokenOfDigitalMoneyAddress addrTo(ckidTo);
    ...
    // -- Parse TokenOfDigitalMoney address
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (Params().NetworkID() == CChainParams::MAIN) {
        devopaddress = CBitcoinAddress("F7CCbdxwD1VZNo8S3uCxUQZyhvNRx3nSmH");
    ...
    if(hasPayment){
        ...
        CTokenOfDigitalMoneyAddress address2(address1);
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CTokenOfDigitalMoneyAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CTokenOfDigitalMoneyAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CTokenOfDigitalMoneyAddress(address).ToString());
}
...
bool CWallet::ImportPrivateKey(CTokenOfDigitalMoneySecret vchSecret, string strLabel, bool fRescan) {
    if (fWalletUnlockStakingOnly)
        return false;
